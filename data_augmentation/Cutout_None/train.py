...
import os
model_options = ['resnet18', 'wideresnet', 'resnet50','vgg']
...
filename = '/cfs/cfs-31b43a0b8/personal/qiangwan/hw/logs' + test_id + '.csv'
...
writer.add_graph(cnn, input_tensor)
...
def test(loader, epoch):
    ...
    writer.add_scalar('Test/acc', float(val_acc), epoch)
    cnn.train()
    return val_acc


logname = 'iter_' + test_id +'_'+ str(args.epochs) +'.csv'
f = open(logname, 'w', encoding='utf-8')
# 2. 基于文件对象构建 csv写入对象
csv_writer = csv.writer(f)
# 3. 构建列表头
csv_writer.writerow(["train_xentropy", "train_acc"])
...
for epoch in range(args.epochs):
    ...
    for i, (images, labels) in enumerate(progress_bar):
        ...
        n_iter = (epoch - 1) * len(progress_bar) + i + 1
        writer.add_scalar('Train/loss', xentropy_loss.item(), n_iter)
        writer.add_scalar('Train/acc', float(accuracy), n_iter)
        csv_writer.writerow([(xentropy_loss_avg / (i + 1)), accuracy])
...
torch.save(cnn.state_dict(), '/cfs/cfs-31b43a0b8/personal/qiangwan/hw/' + test_id + '.pt')
csv_logger.close()
# 5. 关闭文件
f.close()
writer.close()
